...
describe('CanvasResizeComponent', () => {
    ...
    it('should not be smaller than 250px', () => {
        component.resizeCanvas(MIN_SIZE - 1, MIN_SIZE - 1);
        expect(service.canvas.width).toEqual(MIN_SIZE);
        expect(service.canvas.height).toEqual(MIN_SIZE);
    });
    ...
    it('should draw white pixels', () => {
        component.resizeCanvas(MIN_SIZE+1, MIN_SIZE+1);
        const pixelBuffer = service.baseCtx.getImageData(MIN_SIZE, MIN_SIZE, 1, 1).data;
    ...
    it('should resize when dragging the bottom side', () => {
        ...
        dragAndDrop(xPos,yPos,xPos+SHIFTING,yPos+SHIFTING);
        expect(service.canvas.height).toBe(yPos - component.getCanvasTop() + SHIFTING);
    });
    ...
    it('should resize when dragging the right side', () => {
        ...
        dragAndDrop(xPos, yPos, xPos+SHIFTING, yPos+SHIFTING)
        expect(service.canvas.width).toBe(xPos - component.getCanvasLeft() + SHIFTING);
    });
    ...
    it('should resize when dragging the corner', () => {
        ...
        expect(service.canvas.width).toBe(MIN_SIZE);
        expect(service.canvas.height).toBe(MIN_SIZE);
    });
